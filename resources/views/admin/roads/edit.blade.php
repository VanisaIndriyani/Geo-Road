...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-3" data-aos="fade-up">
        <div class="text-dark">
            <div class="h4 fw-bold mb-0">Edit Data Jalan</div>
    ...
    <form method="POST" action="{{ route('admin.roads.update', $road) }}" enctype="multipart/form-data" class="row g-3 g-lg-4">
        @csrf
        @method('PUT')
        ...
        <div class="col-lg-5" data-aos="fade-up">
            <div class="gr-card p-4">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control rounded-4 @error('nama_ruas') is-invalid @enderror" name="nama_ruas" value="{{ old('nama_ruas', $road->nama_ruas) }}" required>
                ...
                <div class="row g-2">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control rounded-4 @error('kabupaten') is-invalid @enderror" name="kabupaten" value="{{ old('kabupaten', $road->kabupaten) }}" required>
                        @error('kabupaten')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-semibold">Kecamatan</label>
                        <input type="text" class="form-control rounded-4 @error('kecamatan') is-invalid @enderror" name="kecamatan" value="{{ old('kecamatan', $road->kecamatan) }}" required>
                ...
                <div class="row g-2">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control rounded-4 @error('panjang') is-invalid @enderror" name="panjang" value="{{ old('panjang', $road->panjang) }}" required>
                        @error('panjang')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-semibold">Lebar Jalan (m)</label>
                        <input type="number" step="0.01" class="form-control rounded-4 @error('lebar') is-invalid @enderror" name="lebar" value="{{ old('lebar', $road->lebar) }}">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select rounded-4 @error('kondisi') is-invalid @enderror" name="kondisi" required>
                        @foreach ($kondisiOptions as $opt)
                            <option value="{{ $opt }}" @selected(old('kondisi', $road->kondisi) === $opt)>{{ $opt }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control rounded-4 @error('jenis_kerusakan') is-invalid @enderror" name="jenis_kerusakan" rows="2" placeholder="Opsional">{{ old('jenis_kerusakan', $road->jenis_kerusakan) }}</textarea>
                ...
                <div class="row g-2">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select rounded-4 @error('prioritas') is-invalid @enderror" name="prioritas">
                            <option value="">-</option>
                            <option value="Rendah" @selected(old('prioritas', $road->prioritas) === 'Rendah')>Rendah</option>
                            <option value="Sedang" @selected(old('prioritas', $road->prioritas) === 'Sedang')>Sedang</option>
                            <option value="Tinggi" @selected(old('prioritas', $road->prioritas) === 'Tinggi')>Tinggi</option>
                        </select>
                        @error('prioritas')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-semibold">Tahun Survey</label>
                        <input type="number" class="form-control rounded-4 @error('tahun') is-invalid @enderror" name="tahun" value="{{ old('tahun', $road->tahun) }}" placeholder="2026">
                ...
                <div class="mb-3">
                    ...
                    @if ($road->foto_url)
                        <div class="mt-2 d-flex align-items-center gap-2">
                            <img src="{{ $road->foto_url }}" alt="Foto" class="rounded-3" style="width:92px;height:66px;object-fit:cover">
                            <div class="small text-muted">Foto saat ini</div>
                        </div>
                    @endif
                </div>

                <input type="hidden" name="geometry" id="geometry" value="{{ old('geometry', $road->geometry) }}">
            ...
            <div class="gr-card p-4">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                    <div>
                        ...
                        <div class="small text-muted">Edit polyline ruas jalan pada peta.</div>
                    </div>
